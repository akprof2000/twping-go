// Команда twping измеряет двусторонние задержки до сервера TWAMP
// (RFC 5357).
//
...
import (
	...
	"github.com/akprof2000/twping-go/internal/owamp"
)
...
func run() error {
	...
	args := normalizeVerbose(os.Args[1:])
	...
	cntrl, err := owamp.OpenControl(owamp.ControlConfig{
		Server:       remoteServ,
		LocalAddr:    srcAddr,
		Network:      network,
		OfferedModes: modes,
	...
	if !o.quiet {
		...
		fmt.Printf("Результаты будут доступны примерно через %.1f с\n", remaining.Seconds())
	}
